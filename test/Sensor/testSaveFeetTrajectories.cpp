...
main (int argc, char** argv)
{
 ...
 //------------- INIT -----------------------------------------//
  ...
  float clusterTolerance (0.02); // min dist between two cluster
 ...
 // double confidence(0.02);       // confidence for plane detection
  ...
  double x,y,theta;
   ...
   // ----- Filename to save the feet position-----------------------//

   string filenameFeet = path +"/"+"trajectories.dat";
   std::cout << "Log file is : "<<filenameFeet << std::endl;
   std::ofstream fileFeet(filenameFeet.c_str(),ios::out); 
   ...
   for(int index=nbSampleBegin ; index<=nbSampleEnd ; index+=sampleStep ){
    ...
    // test si on a changé d'image. Si on n'a pas changé d'image. Il ne 
    // sert à rien de refaire tous les traitement et en plus, on ne déplace pas les pieds mais
    // uniquement le déambulateur
    // si on change d'image alors on fait le traitement et on fait avancer les pieds
    // sinon les pieds glissent et c'est n'importe quoi
    if(nbIm == nbImPrec)
      newImage = false;
    else
    {
      ...
      nbImPrec = nbIm;
      // say it loud
      std::cout << "New image ------------> treat ! "<< std::endl;
    }
	...
	if(fileFeet){
      		fileFeet << setprecision(precision) << currtime << "\t" //1
                   << setprecision(precision) << x << "\t"   //2
                   << setprecision(precision) << y << "\t"	 //3
                   << setprecision(precision) << theta << "\t"	//4 
                   << setprecision(precision) << foot1.getLabel() << "\t"//5	 
                   << setprecision(precision) << foot1.getTX() << "\t"	 //6
                   << setprecision(precision) << foot1.getTY() << "\t"	 //7
                   << setprecision(precision) << foot1.getTZ() << "\t"	 //8
                   << setprecision(precision) << foot1.getRX() << "\t"	 //9
                   << setprecision(precision) << foot1.getRY() << "\t"	 //10
                   << setprecision(precision) << foot1.getRZ() << "\t"	//11
                   << setprecision(precision) << foot1.getToeX() << "\t"	 //12
                   << setprecision(precision) << foot1.getToeY() << "\t"	 //13
                   << setprecision(precision) << foot1.getToeZ() << "\t" //14
                   << setprecision(precision) << foot2.getLabel() << "\t"	 //15
                   << setprecision(precision) << foot2.getTX() << "\t"	 //16
                   << setprecision(precision) << foot2.getTY() << "\t"	 //17
                   << setprecision(precision) << foot2.getTZ() << "\t"	 //18
                   << setprecision(precision) << foot2.getRX() << "\t"	 //19
                   << setprecision(precision) << foot2.getRY() << "\t"	 //20
                   << setprecision(precision) << foot2.getRZ() << "\t"	//21
                   << setprecision(precision) << foot2.getToeX() << "\t"	 //22
                   << setprecision(precision) << foot2.getToeY() << "\t"	 //23
                   << setprecision(precision) << foot2.getToeZ() << "\n"; //24
			}
  else 
    {
