...
main (int argc, char** argv)
{
 ...
 //------------- INIT -----------------------------------------//
  ...
  kinect->extrinsicCalibration(pointCloud,confidence,leafSize);    
  ...
  Eigen::MatrixXf ptsIn(gPointCloud), ptsOut(1,3) ;
  double distThreshold(2*confidence);
  ...
  Conversion::convert(ptsOut,cloudFeet);
  ...
  kinect->filter(cloudFeet,cloudFeetFiltered, leafSize);

  // segment the point filtered point cloud
  kinect->segment(cloudFeetFiltered,clusterIndices,minClusterSize, maxClusterSize, clusterTolerance);
